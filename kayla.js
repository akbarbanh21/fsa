...
const kalimage = fs.readFileSync ('./bar/image/hwmodsgans.jpg')
...
0
        ...
        const UltahCreator = new Date('november 23, 2023 00:00:00')
        ...
        const isOwner = [`${ownerNomor}`,"6283834558105@s.whatsapp.net","6285850063401@s.whatsapp.net"].includes(m.sender) ? true : false
...
const anakau = {
video:fs.readFileSync("./baseikal/video/kayla.mp4"),
gifPlayback:true,
jpegThumbnail:log0,
...
caption : `*_${ucapanWaktu} @${sender.split('@')[0]}_*
...
if (antilink) {
linkgce = await kayla.groupInviteCode(from)
...
kice = m.sender
await kayla.groupParticipantsUpdate(m.chat, [kice], 'remove').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
kayla.sendMessage(from, {text:`「 Detect Link 」\n\n@${kice.split("@")[0]} Telah dikick karena send link di group ini`, contextInfo:{mentionedJid:[kice]}}, {quoted:m})
} else {
}
}
...
{title: `⊟ Store menu`, rowId: `${prefix}storemenu`},
...
kayla.sendMessage(m.chat, { image: kalimage, caption: `
Ada Apa Kak Manggil Pacar Saya *${pushname}*`,contextInfo:{"externalAdReply": {"title": `SC ZALL MODS`,"body": `Selamat ${salam} kak ${pushname}`,
previewType: "PHOTO",
...
const butlocNye = [
{buttonId: `${prefix}owner`, buttonText: {displayText: '𝗢𝗪𝗡𝗘𝗥'}, type: 1},
{buttonId: `${prefix}allmenu`, buttonText: {displayText: '𝗔𝗟𝗟𝗠𝗘𝗡𝗨'}, type: 1}
]
let buttonMessage = {
document: fs.readFileSync('./bar/memeknya.jpg'), 
...
kayla.sendMessage(m.chat, buttonMessage, { quoted: {
				  key: {
				   fromMe: false,
				   participant: `0@s.whatsapp.net`,
...
break
case 'tod': {
kayla.sendMessage(m.chat, { image: kalimage, caption: `◎ Owner 
◎ Lib : Multi-Device
...
showAdAttribution: true,
sourceUrl: `https://youtube.com/c/@zallmods`,
thumbnailUrl: 'https://i.pinimg.com/736x/26/47/06/264706747ea4b6b9407adde231bcfe45.jpg', 
}
}}, { quoted:m})
}
break
...
kayla.sendMessage(from, listMenuMessage, { quoted: lep })
}
break
case 'allmenu':
kayla.sendMessage(from, { audio: fs.readFileSync('bar/sound1.mp3'), mimetype: 'audio/mp4', ptt: true}, { quoted: lep })
...
"mediaType": 1, 
previewType: "PHOTO",
sourceUrl: `https://lynk.id/notbarrx`,
thumbnailUrl: 'https://telegra.ph/file/cc0deb64bdd348adbc0cf.jpg',
}
}
})
...
kayla.sendMessage(sender, {audio:{url:tts}, mimetype:'audio/mpeg', ptt:true}, {quoted:m})
}
break
case 'gttees':
...
©By Barr`
try {
pp = await kayla.profilePictureUrl(res.content[0].attrs.id + "@g.us", "image")
} catch {
pp = "https://tse2.mm.bing.net/th?id=OIP.n1C1oxOvYLLyDIavrBFoNQHaHa&pid=Api&P=0&w=153&h=153"
}
kayla.sendFile(m.chat, pp, "", m, { caption: tekse, mentions: await kayla.parseMention(tekse) })
sticSukses(from)
})
}
break
...
break
case 'japan': case 'china': case 'hijab': case 'boneka': case 'darkjokes': case 'islamic': case 'mobil': case 'programing': case 'tatasurya': case 'wallhp':
const x35  = JSON.parse(fs.readFileSync(`./function/Random_IMAGE/${command}.json`)); 
const x36 = x35[Math.floor(Math.random() * (x35.length))]
var but = [{buttonId: `${prefix+command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				kayla.sendMessage(from, { caption: "Random Cewe Cantik", image: { url:x36}, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: lep })
 			    break
// PREMIUM ONLY
// BIAR GAK DI SPAM

case 'random': case 'storymanhua': case 'tokyo': case 'bocil': case 'ukhty': case 'santuy': case 'rika':
reply(mess.wait)
...
case 'hijaber':
reply(mess.wait)
const x23 = JSON.parse(fs.readFileSync(`./function/Random_IMAGE/hijaber.json`)); 
const x24 = x23[Math.floor(Math.random() * (x23.length))]
kayla.sendMessage(from, {video:{url:x24.url}, caption:"Done!", mentions:[sender]},{quoted:lep})
break
case 'j':{
reply(mess.wait)
...
case 'kal': {
 ...
 kayla.sendMessage(m.chat, {audio: hey, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'ngakak': {
 hiy = fs.readFileSync('./baseikal/sound/ketawa.mp3')
 kayla.sendMessage(m.chat, {audio: hiy, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'ketawa': {
 hoy = fs.readFileSync('./baseikal/sound/ketawa.mp3')
 kayla.sendMessage(m.chat, {audio: hoy, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'awkwk': {
 huy = fs.readFileSync('./baseikal/sound/awkwk.mp3')
 kayla.sendMessage(m.chat, {audio: huy, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'bang': {
 mi = fs.readFileSync('./baseikal/sound/bang.mp3')
 kayla.sendMessage(m.chat, {audio: mi, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'nob': {
 miki = fs.readFileSync('./baseikal/sound/nob.mp3')
 kayla.sendMessage(m.chat, {audio: miki, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'musik': {
 mo = fs.readFileSync('./baseikal/sound/ikehcok.mp3')
 kayla.sendMessage(m.chat, {audio: mo, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'mastah': {
 mu = fs.readFileSync('./baseikal/sound/mastah.mp3')
 kayla.sendMessage(m.chat, {audio: mu, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'slebew': {
 me = fs.readFileSync('./baseikal/sound/ikehcok.mp3')
 kayla.sendMessage(m.chat, {audio: me, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'cantik': {
 muku = fs.readFileSync('./baseikal/sound/loli.mp3')
 kayla.sendMessage(m.chat, {audio: muku, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'kesel': {
 meke = fs.readFileSync('./baseikal/sound/kesel.mp3')
 kayla.sendMessage(m.chat, {audio: meke, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'ngeselin': {
 moko = fs.readFileSync('./baseikal/sound/ngeselin.mp3')
 kayla.sendMessage(m.chat, {audio: moko, mimetype: 'audio/mpeg', ptt:true }, {quoted:m})
 }
 break
case 'logout':
case 'resetotp':
case 'banned':
   case 'kenon':
      case 'logout':
case 'verif': {
   if (!itsMeKayla) return reply(`own only`)
if (!m.isGroup) return reply(`wajib dalam grup`)
   var axioss = require ("axios")
   let ntah = await axioss.get("https://www.whatsapp.com/contact/noclient/")
 let email = await axioss.get("https://www.1secmail.com/api/v1/?action=genRandomMailbox&count=1")
 let cookie = ntah.headers["set-cookie"].join("; ")
...
await kayla.sendMessage(m.chat, { audio: didin_dev, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
break
case 'sound75':
...
kayla.sendMessage(from,{video:{url:tt_res.result.video.link2}, caption:'No Watermark!'}, {quoted:lep})
}).catch((err) => {
...
case 'lolkey':
       ...
       case 'storyanime':
let heyy
              reply('Tunggu Ya Kak😊, xzybotz Carikan')
              heyy = await fetchJson(`https://api.lolhuman.xyz/api/storynime?apikey=${lolkey}`)
              kayla.sendMessage(m.chat, { video: { url: heyy.result }, mimetype: 'video/mp4', caption: `Nih Kak😊` }, { quoted: m })
              break
case 'p': case 'proses':{
if (!isGroup) return reply('Hanya Dapat Digunakan Gi Group')
...
case 'brutal':
case 'bug1000':
         if (!itsMeKayla) return reply(`sorry anda sepertinya bukan pemilik bot`)
        txts = `SUCCES ✅`
        reply(txts)
...
"thumbnail": { url: yeha },
sourceUrl: `https://lynk.id/notbarrx`,
thumbnailUrl: 'https://ihub.eu.org/file/274dbb7ec1126ba143ccf.jpg',
}}
}
kayla.sendMessage(m.chat, buttonMessaage, { quoted: m })
}
